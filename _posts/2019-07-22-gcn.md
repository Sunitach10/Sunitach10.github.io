...
 D = np.matrix(np.diag(D))
 '''
Normalizing A such that all rows sum to one, i.e. D-1A, where D is the diagonal node degree matrix, gets rid of this problem. Multiplying with D -1A , now corresponds to taking the average of neighboring node features. turning the aggregate into a mean where the scale of the aggregated feature representation is invariant to node degree.
